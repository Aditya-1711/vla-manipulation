...
print("=========================================================")
print("=== 1. CONSTRUCTING POLICY + PRE/POST PROCESSOR PIPELINE ===")
print("=========================================================")
...
print(f"Action mean: {ds_meta.stats['action']['mean']}")
print(f"Action std:  {ds_meta.stats['action']['std']}")
...
try:
    ...
    print("Pre/Post processors created successfully via make_default_pre_post_processors!")
except Exception as e:
    print(f"make_default_pre_post_processors error: {e}, falling back to make_policy built-ins...")
    preprocessor = getattr(policy, "preprocessor", None)
...
print("\n=========================================================")
print("=== 2. SINGLE EPISODE ROLLOUT (SEED 1000) WITH PROCESSOR PIPELINE ===")
print("=========================================================")
...
for step in range(15):
    ...
    with torch.no_grad():
        # Step A: Apply Preprocessor (if available)
        proc_obs = preprocessor(raw_obs) if preprocessor is not None else raw_obs
        
        # Step B: Policy select_action
        raw_action = policy.select_action(proc_obs)
        
        # Step C: Apply Postprocessor (if available)
        final_action = postprocessor(raw_action) if postprocessor is not None else raw_action

...
print("=========================================================")
print("=== 1. LOADING MODEL WEIGHTS SAFELY WITH DEVICE='CPU' ===")
print("=========================================================")
...
cfg.device = "cpu"  # Keep device = "cpu" inside config so safetensors loads cleanly to RAM

# Load policy with device="cpu"
policy = make_policy(cfg=cfg, ds_meta=ds_meta, rename_map={"observation.image": "observation.images.camera1"})
policy.eval()
print(f"Policy loaded on CPU!")
...
print("Pre/Post processors initialized successfully on CPU!")

# Move model to GPU using torch.device("cuda") AFTER safetensors loading finishes
if torch.cuda.is_available():
    policy.to("cuda")
    print(f"Policy transferred to CUDA!")

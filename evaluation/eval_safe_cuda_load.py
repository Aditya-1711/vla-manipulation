...
# 1. Force CPU-side Safetensors loading & synchronous CUDA error reporting for debugging
...
print("=========================================================")
print("=== 1. SAFE CPU LOADING OF POLICY & PROCESSORS ===")
print("=========================================================")
...
cfg.device = "cpu"  # Load initial weights onto CPU safely

# Load policy on CPU to avoid CUDA memory access error during safetensors deserialize
policy = make_policy(cfg=cfg, ds_meta=ds_meta, rename_map={"observation.image": "observation.images.camera1"})
policy.eval()

# Construct processors on CPU
...
# Move policy to CUDA after clean CPU loading
target_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
if torch.cuda.is_available():
    policy = policy.to("cuda")
    print(f"Policy successfully transferred to CUDA device: {torch.cuda.get_device_name(0)}")

tokenizer = AutoTokenizer.from_pretrained("HuggingFaceTB/SmolVLM2-500M-Video-Instruct")
tokenizer_out = tokenizer(TASK_INSTRUCTION, return_tensors="pt", padding=True)
lang_tokens = tokenizer_out["input_ids"].to(target_device)
lang_mask = tokenizer_out["attention_mask"].bool().to(target_device)
...
print("\n=========================================================")
print("=== 2. SINGLE EPISODE ROLLOUT (SEED 1000) WITH SAFE DEVICE HANDSHAKE ===")
print("=========================================================")
...
for step in range(15):
    ...
    img_tensor = img_tensor.unsqueeze(0).to(target_device)
    ...
    state_tensor = torch.from_numpy(tcp_pose_7d).float().unsqueeze(0).to(target_device)
